...
- En användares applikationsaccess lagras inte separat utan härleds som unionen av applikationerna för användarens grupper. Gruppformuläret äger kopplingen och användarvyn visar resultatet skrivskyddat.
...
- Vid SAML-svar samt import/export av `users.js` adapteras gruppmedlemskapet till det befintliga claim-formatet `groups: "grupp-a,grupp-b"`. Gruppnamn får därför inte innehålla kommatecken. Exporterad applikationsaccess är härledd metadata, aldrig ett SAML-claim; äldre användarbaserad metadata importeras bara när samma access kan uttryckas exakt med grupperna.
